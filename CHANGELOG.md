...
### Added
- Model catalog on both surfaces: `GET /api/settings/models?route=<route>` asks the route's provider which model ids it actually serves (OpenAI-compatible `GET /models`, or the Anthropic list endpoint), sorted and deduped, with provider failures returned as `200 {ok:false, error}` like test-connection. The Settings panel's model fields now carry a datalist fed by that endpoint, scoped to the focused row's selected route and cached per route, so the ids behind the gateway (Claude, GPT, local Ollama) are pickable instead of typed from memory; free text still works. MCP clients get the same catalog via a new `warroom_list_models` tool on both transports, closing the loop with `warroom_set_model` / `warroom_test_model`. Covered by endpoint tests, an MCP-client test, and a Playwright E2E proving the datalist swaps with the row's route.
