...
### Added
- A model change is dry-run before it is stored. Setting a model used to be a blind write on both surfaces: the panel and `warroom_set_model` accepted any string, and the first proof that the pair was wrong arrived as a dead agent turn in the middle of a deliberation. The per-pair Test button only covered the pair you happened to type, and only a plain completion. `POST /api/settings/preflight` and the new `warroom_preflight` tool now walk the pipeline the way the server will run it: all five phases with their agents, every specialist template, and the five support calls that bracket the phases (fingerprint classifier, memory analyzer, problem improver, adversarial twin, quality evaluator), which no operator ever repoints because they are not routable, and which is how a session reaches synthesis and then fails to score itself. Each checkpoint resolves against the CANDIDATE routing rather than the stored one, and one real minimal completion fires per distinct (route, model, kind) pair, so the common case of every agent on one model costs two round trips, not thirty-one. Two probe kinds because the deliberation uses two transports: `chat` for the support calls, and `tools` for every agent turn, which always ships a tools array (`escalate_to_human`, plus `web_search` for search agents) and so fails on a model that answers a bare prompt but rejects `tools`. A route configured without credentials counts as a failure rather than a silent fallback to the default provider. `PUT /api/settings/agent-routing` runs the dry run and answers `409 preflight_failed` with the report instead of storing a configuration that cannot deliberate; `force` stores it anyway, `skipPreflight` stores it without probing, and a write that changes no pair skips the probe entirely. `warroom_set_model` blocks the same way, returns the report as a tool error naming what failed, and tells the caller to fix the id or pass `force`. The Settings panel grows a Dry run button, a per-phase verdict panel carrying the provider's own error text, and a Save anyway button that only appears after a failure. Covered by `tests/preflight.test.mjs` (HTTP gate, probe deduplication, tools-probe proof, MCP blocking) and a Playwright E2E driving all four paths.
- Claude Opus 5 is the deployment default model. `MODEL` still wins where a deployment sets it; the built-in fallback was `anthropic/claude-opus-4-8`.
...
### Fixed
- Sessions stopped mid-run and reported themselves finished. Ten sessions over two days: two reached Synthesis cleanly, three produced zero messages, four stopped partway, and one of those four was stored as `outcome: complete`. Six defects, all engine-side, none of them the caller's polling:
  - **No LLM call had a deadline.** Only `testConnection` and `listModels` carried a timeout; agent turns ran `fetch` with no signal at all, so nothing bounded how long a session could sit silent on a provider that accepts a request and never answers. A legitimate uncapped turn on this deployment takes 60-300s (longest measured 304s), which is exactly why the gap was invisible: a hung call is indistinguishable from a slow one, and no phase advance, log line, or error row would ever arrive. Every call now carries `LLM_TIMEOUT_MS` (default 10 min, comfortably past the slowest real turn) via an explicit `AbortController`, and a timeout ends the turn instead of consuming the retry budget on a dead connection. The deadline covers the response body, not just the headers: `fetch` resolves the moment headers arrive, so clearing the timer there left a gateway that answers `200 OK` and then trickles the JSON hanging exactly as before (measured: still running at 125x the configured window). On the Anthropic transport the SDK's `APIConnectionTimeoutError` sets no `name`, so it was misclassified as transient and retried three times on top of the SDK's own two: nine upstream requests for one agent turn, now one.
  - **Nor did search.** `createTavilyProvider` only armed an abort controller when a caller passed `timeoutMs`, and `server.js` never passed one, so the same unbounded hang was reachable through `web_search`. The deadline is now the provider's own default (20s), not a knob a caller can forget.
  - **`outcome: complete` meant "produced at least one message".** A run killed at Problem Framing by a redeploy was stored identically to one that reached Synthesis, and `mcp/tools.js` compounded it by rendering every inactive session as `Status: Complete`. The outcome now answers "did every phase run": `complete`, `stopped` (ended early), `failed` (no verdict), `crashed` (server restarted mid-run). Reaching the last phase is not finishing it: a single 429 on the synthesis turn leaves a transcript with no verdict, so `complete` now requires the verdict message to exist, which is the same rule migration 022 applies to the historical rows. SIGTERM and both stop paths stamp `stopped` themselves rather than leaving a NULL row that reads as a legacy completion for as long as the in-flight turn lasts, boot reconciliation stamps `crashed`, resume clears the previous label, and only a `complete` run is quality-scored — which is how a three-message session came to carry a score of 0.249. The browser reads the same field: the detail badge, the phase rail, and the history card no longer derive completion from `active` alone, and a circuit-breaker abort no longer renders a success banner and an export prompt. Migration 022 relabels the historical rows that never reached Synthesis.
  - **Answering an escalation over MCP did not wake the room.** `mcp/http.js` persisted the answer and set the in-memory flag but never called `resolveEscalation`, so the deliberation stayed parked for its full five-minute window while the caller believed it had unblocked it. Both transports now share `answerEscalationById`.
  - **A timed-out blocking escalation was never marked answered**, so the next agent turn re-armed a fresh five-minute wait on the same question, and so did every turn after it: one unanswered question could hold a room for over an hour. It now resolves to the agent's stated default, the way the phase gate already resolved an optional one.
  - **A provider refusing every request still cost a full five-phase run.** On 2026-08-11 three sessions each drove all 21 turns into a 402/429 storm in under two minutes and produced nothing. The run is abandoned after `MAX_CONSECUTIVE_TURN_FAILURES` (default 4) consecutive failed turns and recorded as `failed`.
- OpenRouter refused entire agent turns with `402 ... you requested up to 65536 tokens, but can only afford 20722`. Omitting `max_tokens` (correct, and what #52 fixed) makes OpenRouter reserve the model's whole output ceiling against the credit balance, so a low balance fails every request outright rather than producing a shorter answer. The 402 names what the balance covers, so the request is re-issued once at exactly that budget; a second 402 means the balance is genuinely empty and fails fast instead of burning three blind retries.
- `warroom_get_session` surfaces why a run died. Every failed turn already recorded its provider error in `search_metrics` and nothing exposed it, so an MCP caller could not tell a credit refusal from a rate limit from a stall. The payload now carries `failedTurns`, `lastError`, `crashRecoveredAt` and `totalPhases`, and the rendered detail shows the failed-turn count with the last provider error and the phases the run got through.
