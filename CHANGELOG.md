...
### Fixed
- Sessions stopped mid-run and reported themselves finished. Nine sessions over two days: one reached Synthesis, three produced zero messages, five stopped partway, and three of those five were stored as `outcome: complete`. Six defects, all engine-side, none of them the caller's polling:
  - **No LLM call had a deadline.** Only `testConnection` and `listModels` carried a timeout; agent turns ran `fetch` with no signal at all. A provider that accepts a request and never answers therefore parked the turn, its phase, and the whole session, silently and forever: session `3fe5a74f` sat on one OpenRouter request for over two hours, still `active=1`, with no log line, no error row, and no phase advance. Every call now carries `LLM_TIMEOUT_MS` (default 10 min) via an explicit `AbortController` (`AbortSignal.timeout` is unref'd and cannot fire in an idle process), and a timeout ends the turn instead of consuming the retry budget on a dead connection.
