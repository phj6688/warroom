...
3. **Right-rail PENDING escalation panel + bulk action** — escalations accumulate in a panel (not inline interrupts), grouped by agent, rAF-batched render, single shared pulse keyed on mount. Bulk button: `ACCEPT N DEFAULTS · PROCEED` with 5s undo (no modal). `[REFRAME]` tap → inline input + auto-flag blocking. Severity = **intensity of the live red accent** (`#D71921` full = blocking, dimmed = optional) — NOT a new hue. (`public/index.html`)
