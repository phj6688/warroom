...
describe('route forwarding: WS new-session', () => {
  // The WS entry point must forward the link the same way POST does. A child
  // script wires setupWebSocket against a fake wss/ws and a stubbed
  // createSession that records its args; the real validateWS gate runs first.
  test('forwards continuesFromSessionId as the 4th createSession arg', async () => {
    const dbPath = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'wr-wsfwd-')), 'f.db');
    const script = `
      'use strict';
      process.env.WAR_ROOM_DB_PATH = ${JSON.stringify(dbPath)};
      (async () => {
        ...
        try {
          dbMod = require(${JSON.stringify(path.join(REPO_ROOT, 'db.js'))});
          wsMod = require(${JSON.stringify(path.join(REPO_ROOT, 'lib', 'ws-handler.js'))});
        } catch (err) {
          process.stdout.write(JSON.stringify({ ok: false, stage: 'require', error: err.message }));
          process.exitCode = 1; return;
        }
        let captured = null;
        // wss is an EventEmitter; setupWebSocket registers a 'connection' handler.
        const wss = new EventEmitter();
        wsMod.setupWebSocket(wss, {
          stmts: dbMod.stmts, activeSessions: new Map(), AGENTS: [], PHASES: [],
          createSession: async (problem, fileIds, presetId, continuesFromSessionId) => {
            captured = { problem, fileIds, presetId, continuesFromSessionId };
            return { id: 's-new', problem, phase: 0, active: true, createdAt: Date.now() };
          },
          ...
          broadcast: () => {},
          broadcastGlobal: () => {},
          specialist: {}, getAgentsForSession: () => [],
        });
        // A fake ws: EventEmitter for 'message', no-op send, OPEN-equivalent state.
        const ws = new EventEmitter();
        ws.send = () => {};
        ws.readyState = 1; // WebSocket.OPEN
        wss.emit('connection', ws);
        // The handler is async; emit returns synchronously, so await the message
        // handler by listening for its completion via a microtask drain.
        const listeners = ws.listeners('message');
        if (listeners.length === 0) { process.stdout.write(JSON.stringify({ ok:false, stage:'no-message-handler' })); process.exitCode = 3; return; }
        await listeners[0](JSON.stringify({ type: 'new-session', problem: 'hello', continuesFromSessionId: 'prior-1' }));
        process.stdout.write(JSON.stringify({ ok: true, captured }));
      })().catch(err => { process.stdout.write(JSON.stringify({ ok:false, stage:'unhandled', error: err && err.message })); process.exitCode = 9; });
    `;
    const { code, stdout, stderr } = await runNodeScript(script, { timeoutMs: 20_000 });
...
describe('loadSession read-back: continuesFromSessionId', () => {
  // server.js loadSession() rebuilds the in-memory session from the DB row.
  // It must read continues_from_session_id back, or runDeliberation's
  // continuation block (gated on session.continuesFromSessionId) silently
  // skips injection on every resume. server.js loads the row via
  // `SELECT * FROM sessions WHERE id = ?` and maps the field as:
  //   continuesFromSessionId: row.continues_from_session_id || null
  // server.js cannot be required in-process (no main guard; it binds a
  // listener on require), so we assert the column round-trips through the
  // same row shape AND pin the exact mapping expression here. A column rename
  // or a dropped read-back breaks this test.
  test('a continued row maps to the link; a control row maps to null', () => {
    const dbPath = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'wr-loadrb-')), 'c.db');
    runMigrations({ dbPath, migrationsDir: path.join(root, 'migrations') });
    const db = new Database(dbPath);
    const now = Date.now();
    db.prepare('INSERT INTO sessions (id, problem, phase, active, created_at, updated_at) VALUES (?,?,0,1,?,?)').run('src', 'p', now, now);
    ...
    // Mirror loadSession's row read + field mapping exactly.
    const mapLink = (id) => {
      const row = db.prepare('SELECT * FROM sessions WHERE id = ?').get(id);
      return row.continues_from_session_id || null;
    };
    assert.equal(mapLink('cont'), 'src', 'continued row reads its source link');
    assert.equal(mapLink('src'), null, 'control row reads null (no || fallthrough surprise)');
    db.close();
  });
});
