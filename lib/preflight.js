...
function createPreflight(deps) {
  ...
  async function run({ routing = null, timeoutMs = DEFAULT_TIMEOUT_MS, concurrency = DEFAULT_CONCURRENCY } = {}) {
    ...
    for (const cp of checkpoints) {
      ...
      if (cp.fellBack) {
        cp.ok = false;
        // resolveRoute falls back for three different reasons and the operator
        // fixes each differently: add a credential, add a model id, or replace
        // a stored Haiku model, which resolveRoute skips as if it were absent.
