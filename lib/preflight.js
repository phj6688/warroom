...
// The calls that bracket the phases. They are routable like any agent
// (lib/routable-agents.js), but one with no stored row resolves through the
// deployment default, which is how a session reaches synthesis and then fails
// to score itself. So the dry run checks them on every walk.
const SUPPORT_CALLS = [
  ...
  async function run({ routing = null, timeoutMs = DEFAULT_TIMEOUT_MS, concurrency = DEFAULT_CONCURRENCY } = {}) {
    ...
    function makeCheckpoint(agentId, kind, meta) {
      ...
      return {
        ...
        requestedModel: (want && want.model) || null,
    ...
    for (const cp of checkpoints) {
      ...
      // resolveRoute skips a stored Haiku model as if it were absent, so the
      // agent runs on a model the stored row does not name. That is a silent
      // fallback even when the row has no route, and fellBack only sees routes.
      if (isHaikuModel(cp.requestedModel)) {
        cp.ok = false;
        cp.error = `model ${cp.requestedModel} is skipped (${HAIKU_REFUSAL}); this agent would run on ${cp.model} via ${cp.route} instead`;
        continue;
      }
      if (cp.fellBack) {
        cp.ok = false;
        // resolveRoute falls back for two different reasons and the operator
        // fixes them differently: add a credential, or add a model id.
        const why = cp.requestedModel
          ? `has no credentials on this server`
          : `carries no model id, and a non-default route needs one`;
        cp.error = `route ${cp.requestedRoute} ${why}; this agent would silently run on ${cp.route} instead`;
        continue;
      }
      cp.ok = p.ok;
      cp.error = p.error;
