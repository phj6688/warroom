...
function createTokenLedger() {
  const bySession = new Map(); // sessionId -> { cats, byModel: Map<"route::model", bucket> }

  function tallyFor(sessionId) {
    let t = bySession.get(sessionId);
    if (!t) { t = { cats: emptyBreakdown(), byModel: new Map() }; bySession.set(sessionId, t); }
    return t;
  }
   ...
   * than a provider-reported count. When the caller knows which model/route
   * served the call (opts.model), the usage is also tallied per (route, model)
   * for HLB-337 cost attribution.
   */
  function add(sessionId, category, usage, opts = {}) {
    ...
    if (opts.model) {
      const key = `${opts.route || 'default'}::${opts.model}`;
      let m = t.byModel.get(key);
      if (!m) { m = { input_tokens: 0, output_tokens: 0, total_tokens: 0, calls: 0 }; t.byModel.set(key, m); }
  ...
  /**
   * Read-only view: { total_tokens, token_breakdown, by_model }. Unknown
   * sessions return a fully zeroed shape rather than throwing, so the completion
   * path can always persist something. by_model keys are "route::model".
   */
  function snapshot(sessionId) {
    const t = bySession.get(sessionId);
    const cats = t ? t.cats : emptyBreakdown();
    const token_breakdown = {};
    let total = 0;
    for (const c of CATEGORIES) {
      token_breakdown[c] = { ...cats[c] };
      total += cats[c].total_tokens;
    }
    const by_model = {};
    if (t) for (const [k, v] of t.byModel) by_model[k] = { ...v };
    return { total_tokens: total, token_breakdown, by_model };
  }
...
function persistSessionTokens(db, sessionId, ledger, opts = {}) {
  ...
  try {
    if (cost) {
      // HLB-337 — write token + cost columns together (cost columns added in
      // migration 020).
      db.prepare('UPDATE sessions SET total_tokens = ?, token_breakdown = ?, total_cost_usd = ?, cost_breakdown = ?, updated_at = ? WHERE id = ?')
        .run(snap.total_tokens, JSON.stringify(snap.token_breakdown), cost.total_cost_usd, JSON.stringify(cost.cost_breakdown), Date.now(), sessionId);
    } else {
