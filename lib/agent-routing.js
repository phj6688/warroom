...
// Operator ruling: War Room never runs a Claude Haiku model. Every Haiku id
// carries the word, dated or vendor-prefixed alike, so one case-insensitive
// match covers them. The match is broad on purpose: a gateway alias such as
// `haiku` must not slip through a stricter Claude-prefix pattern. lib/llm.js
// applies the same test to env and stored models.
const HAIKU_REFUSAL = 'Claude Haiku models are not allowed on War Room';
...
function validateEntry(agentId, cfg, routes) {
  ...
  if (cfg.model != null) {
    ...
    if (isHaikuModel(model)) return { error: `agent ${agentId}: ${HAIKU_REFUSAL}` };
...
// JSON.stringify of the raw objects. Both write paths use this to decide
// whether a dry run has anything new to prove.
...
module.exports = { validateEntry, sanitizeRouting, mergeRouting, sameRouting, isHaikuModel, HAIKU_REFUSAL, MAX_MODEL_LEN };
