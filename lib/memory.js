...
function createMemoryManager(deps) {
  ...
  async function storeSessionMemory(sessionId) {
    ...
    if (!embedding) {
      console.warn(`⚠️  Could not embed session ${sessionId} — Ollama unavailable`);
      return false;
    }
    ...
    console.log(`📝 Session ${sessionId} embedded (rowid=${rowid})`);
    return true;
  }
  ...
  async function retrieveSimilar(problemText, limit = 3) {
    ...
    try {
      ...
      if (elapsed > 500) {
        console.warn(`⚠️  Memory retrieval took ${elapsed}ms (threshold 500ms)`);
      }
      ...
      console.warn(`⚠️  Memory retrieval error: ${err.message}`);
      return [];
    }
  }
  ...
  async function extractArchivalFacts(sessionId) {
    ...
    try {
      ...
      console.warn(`⚠️  MemoryAnalyzer failed for ${sessionId}: ${err.message}`);
      return null;
    }
  }
  ...
  async function bootstrap() {
    ...
    for (const session of sessions) {
      ...
      if (success) {
        ...
        if (facts) {
          console.log(`  📋 ${session.id}: archetype="${facts.archetype || 'unknown'}"`);
        }
      } else {
        failed++;
      }
    ...
    console.log(`\n📊 Bootstrap complete: ${embedded} embedded, ${skipped} skipped, ${failed} failed`);
