...
function createMemoryManager(deps) {
  const { db, stmts, callAnthropic, AGENTS, PHASES, onTokenUsage } = deps;
  // HLB-152 — attribute the tokens this manager spends to the session it is
  // working on. A no-op when the host did not wire a ledger (e.g. the bootstrap
  // entry point below). The embedding provider (Ollama) does not return a token
  // count, so embedding figures use the char-count estimate and are labelled
  // estimated.
  const recordUsage = (sessionId, category, usage, opts) => {
    if (typeof onTokenUsage === 'function') onTokenUsage(sessionId, category, usage, opts);
  };
  ...
  async function storeSessionMemory(sessionId) {
    ...
    if (!embedding) {
      log.warn({ sessionId }, 'could not embed session — Ollama unavailable');
      return false;
    }
    // The embedding provider returns no usage; estimate input tokens from the
    // embedded text and label the figure estimated.
    recordUsage(sessionId, 'embedding', { input_tokens: estimateTokens(summary), output_tokens: 0 }, { estimated: true });
  ...
  async function extractArchivalFacts(sessionId) {
    ...
    try {
      const response = await callAnthropic(
        ...
        'memory-analyzer',
        800,
        (u) => recordUsage(sessionId, 'memory', u)
      );
