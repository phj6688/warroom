...
const { resolveEscalation, abortSessionWaits, pauseEscalation, resetEscalation, getDeadline, DEFAULT_TIMEOUT_MS } = require('./escalation');
      ...
      try {
        switch (msg.type) {
          ...
          case 'escalation-response': {
            // Inactive/archived sessions are not in activeSessions, so fall
            // back to loadSession — otherwise the user's answer is silently
            // dropped and the UI never updates.
            ...
            if (esc) {
              ...
              // No-op for inactive sessions (no waiter registered).
          ...
          case 'escalation-timer': {
            ...
            // `reset` restarts the window. The live blocking wait (when one is
            // parked) is the source of truth, so we mutate it via the escalation
            // module; we also mirror the state onto the in-memory escalation so
            // the broadcast and any wait that starts LATER honor it.
            let session = activeSessions.get(msg.sessionId);
            if (!session) session = loadSession(msg.sessionId);
            if (!session) break;
            const esc = session.escalations.find(e => e.id === msg.escalationId);
            ...
            if (msg.op === 'pause') {
              ...
              esc.paused = true;
              // Freeze the displayed deadline at its current value.
              if (typeof esc.deadlineAt !== 'number') {
                const t = escalationTiming ? escalationTiming(esc) : getDeadline(session.id, esc.id);
              ...
              // the window from now (a wait that starts later inherits it).
            ...
            if (session) {
              ...
              ws.send(JSON.stringify({
                ...
                session: {
                  ...
                  escalations: (session.escalations || []).map(e => (escalationTiming ? { ...e, ...escalationTiming(e) } : e)),
