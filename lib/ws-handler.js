        ...
        switch (msg.type) {
            ...
            if (session) {
              ...
              ws.send(JSON.stringify({
                ...
                session: {
                  ...
                  // The detail view's badge reads this: an inactive session is
                  // only "Complete" when the outcome says so.
          ...
          case 'stop-session': {
            ...
            if (session) {
              ...
              // Stamp it now. The loop writes the authoritative outcome only
              // when its in-flight turn returns, which can be minutes; until
              // then a NULL outcome reads as a legacy completion on every
              // surface. The loop may upgrade this to 'complete' if the stop
              // landed during the final turn.
              stmts.updateSessionOutcome.run('stopped', null, Date.now(), session.id);
          ...
          case 'resume-session': {
            ...
            // Clear how the last attempt ended (parity with lib/resume.js):
            // a running room must not still report the previous outcome.
            stmts.updateSessionOutcome.run(null, null, Date.now(), resumeSession.id);
