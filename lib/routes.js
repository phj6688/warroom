...
function setupRoutes(app, deps) {
  ...
  app.put('/api/settings/cost', (req, res) => {
    ...
    if (body.subscription) {
      ...
      if (typeof s.planPriceUsd !== 'number' || typeof s.allowanceTokens !== 'number' || s.allowanceTokens <= 0) {
        return res.status(400).json({ error: 'subscription needs a numeric planPriceUsd and a positive allowanceTokens' });
      }
      appConfig.set('subscription', { planPriceUsd: s.planPriceUsd, allowanceTokens: s.allowanceTokens, period: s.period || 'month' });
    }
    if (body.electricity) {
      const e = body.electricity;
      if (typeof e.powerWatts !== 'number' || typeof e.tokensPerSec !== 'number' || typeof e.pricePerKwh !== 'number' || e.tokensPerSec <= 0) {
        return res.status(400).json({ error: 'electricity needs numeric powerWatts, a positive tokensPerSec, and pricePerKwh' });
      }
    ...
    if (body.pricing && typeof body.pricing === 'object' && !Array.isArray(body.pricing)) {
      ...
      for (const [k, v] of Object.entries(body.pricing)) {
        if (v && typeof v.input === 'number' && typeof v.output === 'number') clean[k] = { input: v.input, output: v.output };
      }
