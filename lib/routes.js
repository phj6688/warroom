...
const { requireAuth, authPosture } = require('./auth');
...
const { sanitizeRouting, sameRouting } = require('./agent-routing');
const { routableAgents, routableAgentIds } = require('./routable-agents');
const { availableRoutes, resolveRoute, testConnection, listModels, defaultRouteBilling } = require('./llm');
const { createPreflight } = require('./preflight');
const { getSearchConfigForAgent } = require('./agents/search-config');
const { DEFAULT_PRICING, DEFAULT_SUBSCRIPTION, DEFAULT_ELECTRICITY, DEFAULT_ROUTE_BILLING, BILLING_MODES, sanitizeRouteBilling } = require('./cost');
...
function enrichSession(s, stmts) {
  ...
  return {
    ...
    // How the run ended. Without it the browser can only see `active`, and an
    // inactive row reads as a completion whether it reached Synthesis or was
    // killed at Problem Framing.
...
function setupRoutes(app, deps) {
  ...
  // ─── Auth gate (F1 / S01) ───────────────────────────────────
  // Bearer auth on every endpoint reaching this layer. Static assets are
  // served by the express.static handler above and never reach here.
  //
  // Three exempt paths, each with its own reason:
  //   /health, /metrics  probes and a scrape. They carry counts, no session data.
  //   /mcp               carries its own gate, MCP_API_KEY, checked in
  //                      mcp/http.js. Express runs this middleware first, so
  //                      without the exemption an MCP client would need the
  //                      key in the url AND the bearer in a header, which a
  //                      url-only client entry cannot express: the route would
  //                      be unreachable rather than protected.
  //
  // The comparison is exact equality, never a prefix. A prefix match on /mcp
  // also exempts /mcp-anything, which is how a sibling app on this platform
  // turned its own skip rule into a public bypass.
  const AUTH_EXEMPT_PATHS = new Set(['/health', '/metrics', '/mcp']);
  app.use((req, res, next) =>
    AUTH_EXEMPT_PATHS.has(req.path) ? next() : requireAuth(req, res, next)
  );
  ...
  // ─── Health ─────────────────────────────────────────────────
  // `auth` reports the gate posture ("enforced" or "anonymous"). An instance
  // running open is a state an operator has to be able to read off a probe
  // rather than infer from a boot line scrolled out of the log days ago.
  app.get('/health', (req, res) => {
    const sc = stmts.countAllSessions.get().count;
    res.json({ status: "ok", service: "war-room", sessions: sc, activeSessions: activeSessions.size, uptime: process.uptime(), auth: authPosture() });
  });
  app.get('/api/health', (req, res) => {
    const sessionCount = stmts.countAllSessions.get().count;
    res.json({ status: 'ok', service: 'war-room', sessions: sessionCount, activeSessions: activeSessions.size, uptime: process.uptime(), auth: authPosture() });
  });
  ...
  app.get('/metrics', (req, res) => {
    ...
    const lines = [
      ...
      '# HELP war_room_auth_enforced 1 when /api/* and the WebSocket upgrade require a bearer token, 0 when the instance runs open.',
      '# TYPE war_room_auth_enforced gauge',
      `war_room_auth_enforced ${authPosture() === 'enforced' ? 1 : 0}`,
      '',
    ];
  ...
  // The Settings panel and both MCP transports build their agent list from
  // this endpoint, so it must offer specialists too — see lib/routable-agents.js.
  app.get('/api/settings/agent-routing', (req, res) => {
    const agents = routableAgents(AGENTS, specialist, log);
    const effective = {};
  ...
  // Persist per-agent overrides. Body:
  //   { routing: { agentId: { route?, model? } }, force?, skipPreflight? }
  ...
  //
  // A model change is dry-run before it is stored. Writing first and finding
  // out later means the proof arrives as a dead agent turn mid-deliberation,
  // so a candidate that fails preflight is refused with 409 and the report.
  // `force` stores it anyway (the operator has read the failures and is
  // configuring ahead of a provider that is down); `skipPreflight` stores it
  // without probing at all. A write that does not change any pair skips the
  // dry run: there is nothing new to prove.
  app.put('/api/settings/agent-routing', validateBody(httpAgentRoutingBody), async (req, res) => {
    ...
    const unchanged = sameRouting(clean, appConfig.getAgentRouting());
    if (unchanged || req.body.skipPreflight) {
      appConfig.set('agent_routing', clean);
      return res.json({ ok: true, routing: clean, preflight: null, preflightSkipped: unchanged ? 'unchanged' : 'requested' });
    }

    // Express 4 does not catch a rejected async handler, so a throw here would
    // hang the request rather than answer it. The dry run reports provider
    // failures as data, so reaching this catch means the walk itself broke.
    let report;
    try {
      report = await preflight.run({ routing: clean });
    } catch (err) {
      req.log.error({ err: err.message }, 'preflight run failed');
      return res.status(500).json({ error: 'preflight_error', detail: err.message });
    }
    if (!report.ok && !req.body.force) {
      return res.status(409).json({
        error: 'preflight_failed',
        message: 'The dry run could not complete a deliberation on this configuration. Nothing was saved.',
        preflight: report,
      });
    }
    appConfig.set('agent_routing', clean);
    log.info({ ok: report.ok, forced: !report.ok, durationMs: report.durationMs }, 'agent routing saved after preflight');
    res.json({ ok: true, routing: clean, preflight: report, forced: !report.ok });
  });

  // Standalone dry run: walk every phase, every specialist and every support
  // call on a candidate routing (or on what is stored, when `routing` is
  // omitted) and report what would fail. Persists nothing. Always 200 — a
  // provider failure is the answer, not an endpoint failure.
  app.post('/api/settings/preflight', validateBody(httpPreflightBody), async (req, res) => {
    const body = req.body || {};
    let routing = null;
    if (body.routing) {
      const validIds = routableAgentIds(AGENTS, specialist, log);
      const { clean, error } = sanitizeRouting(body.routing, validIds, appConfig.ROUTES);
      if (error) return res.status(400).json({ error });
      routing = clean;
    }
    try {
      res.json(await preflight.run({ routing, timeoutMs: body.timeoutMs }));
    } catch (err) {
      req.log.error({ err: err.message }, 'preflight run failed');
      res.status(500).json({ error: 'preflight_error', detail: err.message });
    }
  });
  ...
  app.get('/api/settings/cost', (req, res) => {
    // routeBilling is what each route resolves to right now: the deployment's
    // answer for `default` (llm.js knows whether its endpoint is a metered API
    // or a subscription gateway), the per-route defaults, then any operator
    // override on top. The panel renders this, so it must be the effective map
    // rather than only what was stored.
    const effectiveBilling = {
      ...DEFAULT_ROUTE_BILLING,
      default: defaultRouteBilling(),
      ...appConfig.get('route_billing', {}),
    };
    res.json({
      ...
      routeBilling: effectiveBilling,
      billingModes: BILLING_MODES,
      defaults: {
        pricing: DEFAULT_PRICING,
        subscription: DEFAULT_SUBSCRIPTION,
        electricity: DEFAULT_ELECTRICITY,
        routeBilling: { ...DEFAULT_ROUTE_BILLING, default: defaultRouteBilling() },
      },
    });
  });
  ...
  app.put('/api/settings/cost', (req, res) => {
    ...
    if (body.routeBilling) {
      // Unknown routes and modes are dropped rather than rejected, so one bad
      // key reverts that route to its default instead of failing the save.
      appConfig.set('route_billing', sanitizeRouteBilling(body.routeBilling, [...appConfig.ROUTES, 'default']));
    }
    res.json({ ok: true });
  });
