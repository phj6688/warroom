...
function enrichSession(s, stmts) {
  return {
    ...
    escalationCount: stmts.countSessionEscalations.get(s.id).count,
    pinned: !!s.pinned,
  };
}
...
function setupRoutes(app, deps) {
  ...
  // MIME allowlist — text-ish payloads only. Anything else is rejected
  // by the fileFilter with an explicit error code translated by the
  // upload-error middleware below.
  ...
  app.post('/api/sessions', (req, res) => {
    ...
    broadcast({ type: 'session-created', session: { id: session.id, problem: session.problem, files: (session.files || []).map(f => ({ name: f.name, size: f.size, type: f.type })), phase: session.phase, active: session.active, createdAt: session.createdAt } });
  ...
  app.post('/api/sessions/:id/resume', (req, res) => {
    ...
    const resumePhase = session.phase || 0;
    session.active = true;
    ...
    broadcast({ type: 'session-resumed', sessionId: session.id, fromPhase: resumePhase });
    runDeliberation(session, resumePhase).catch(err => console.error('Resume error:', err));
  ...
  app.get('/api/sessions/search/semantic', async (req, res) => {
    ...
    try {
      ...
      for (const row of rows) {
        const meta = db.prepare('SELECT session_id FROM embedding_meta WHERE rowid = ?').get(row.rowid);
  ...
  app.get('/api/sessions/:id/export', (req, res) => {
    const { id } = req.params;
    if (!/^[a-z0-9-]{6,64}$/i.test(id)) return res.status(400).json({ error: 'Invalid session id' });
