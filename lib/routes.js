...
function setupRoutes(app, deps) {
  const { db, stmts, AGENTS, PHASES, activeSessions, callAnthropic, createSession, loadSession, runDeliberation, broadcast, broadcastGlobal, memory, quality, specialist, getAgentsForSession, attachFiles } = deps;
  ...
  app.post('/api/sessions', validateBody(httpCreateSessionBody), async (req, res) => {
    const { problem, file_ids } = req.body;
    try {
      const session = await createSession(problem, file_ids || []);
