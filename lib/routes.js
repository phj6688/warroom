...
function setupRoutes(app, deps) {
  ...
  app.put('/api/settings/agent-routing', (req, res) => {
    ...
    for (const [agentId, cfg] of Object.entries(routing)) {
      ...
      if (cfg.model != null) {
        if (typeof cfg.model !== 'string') return res.status(400).json({ error: 'model must be a string under 200 chars' });
        // Validate and decide on the trimmed value so a whitespace-only model
        // ("   ") is treated as absent, not persisted as an empty model that
        // would slip past the non-default-route guard below.
        const model = cfg.model.trim();
        if (model.length > 200) return res.status(400).json({ error: 'model must be a string under 200 chars' });
        if (model) entry.model = model;
      }
      // A non-default route must carry a model. Otherwise resolveRoute() would
      // send the global Anthropic model id to a non-Anthropic endpoint and the
      // call fails. The default route (no entry.route) takes the env model.
      if (entry.route && !entry.model) {
        return res.status(400).json({ error: `agent ${agentId}: a non-default route requires an explicit model` });
      }
  ...
  app.get('/api/sessions/:id/decision-record', (req, res) => {
    ...
    const sessionRow = stmts.getSession.get(id);
    if (!sessionRow) return res.status(404).json({ error: 'Session not found' });
    if (sessionRow.outcome === 'failed') return res.json({ sessionId: id, available: false, outcome: 'failed' });
    const synthesis = stmts.getSynthesisMessages.all(id);
    if (synthesis.length === 0) return res.json({ sessionId: id, available: false, outcome: sessionRow.outcome || null });
    const verdict = synthesis.map(m => m.content).join('\n\n');
    res.json({ sessionId: id, available: true, outcome: sessionRow.outcome || 'complete', problem: sessionRow.problem, verdict });
  });
}
