...
// Two routing maps are the same write when every agent resolves to the same
// pair. Key order is not meaningful, so compare sorted entries rather than
// JSON.stringify of the raw objects.
function sameRouting(a, b) {
  const norm = (m) => JSON.stringify(
    Object.keys(m || {}).sort().map(k => [k, (m[k] || {}).route || null, (m[k] || {}).model || null])
  );
