...
// once per configuration rather than on every boot.
...
function repriceLegacySessions(deps) {
  ...
  // Which mode the deployment's own route resolves to right now, operator
  // override included. The marker records it, so this runs once per
  // configuration rather than once ever: an operator who later declares the
  // default route a subscription still gets the history they are owed.
  const defaultMode = billingForRoute('default', cfg.routeBilling);
  const marker = appConfig.get(MARKER_KEY, null);
  if (marker && marker.mode === defaultMode) {
  ...
  const run = db.transaction(() => {
    ...
    appConfig.set(MARKER_KEY, { mode: defaultMode, at: Date.now(), repriced });
  });
