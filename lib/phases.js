...
function createRouter(config) {
  ...
  return {
    ...
    next(session, reason) {
      ...
      log.info({ from: phases[from].name, to: phases[to].name, reason: reason || 'linear advance' }, 'phase advance');
    ...
    loopTo(targetIdx, reason) {
      if (loopbackCount >= MAX_LOOPBACKS) {
        log.warn({ max: MAX_LOOPBACKS }, 'phase loop-back denied: max reached');
        return false;
      }
      ...
      log.info({ from: phases[from].name, to: phases[targetIdx].name, count: loopbackCount, max: MAX_LOOPBACKS, reason }, 'phase loop-back');
...
/**
 * Walk the phase list and return the index of the first phase whose required
 * agents have not all produced a message yet. Returns PHASES.length when every
 * phase is complete. Used by the resume path so a session that was stopped
 * mid-deliberation picks up where it left off instead of re-running phases
 * that already ran to completion.
 */
function computeResumePhase(session, phases = DEFAULT_PHASES) {
  const messages = (session && session.messages) || [];
  for (let i = 0; i < phases.length; i++) {
    const p = phases[i];
    const covered = new Set(
      messages.filter(m => m.phase === p.name).map(m => m.agentId)
    );
    if (!p.agents.every(a => covered.has(a))) return i;
  }
  return phases.length;
}

module.exports = { PHASES, createRouter, computeResumePhase, DEFAULT_PHASES, MAX_LOOPBACKS };
