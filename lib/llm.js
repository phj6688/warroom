...
const { isHaikuModel, HAIKU_REFUSAL } = require('./agent-routing');

// A model env var that names Claude Haiku counts as unset, so resolution moves
// on to the next source instead of calling a model War Room never runs.
function modelFromEnv(name, value) {
  if (!value) return null;
  if (isHaikuModel(value)) {
    log.warn({ [name]: value }, `${name} ignored: ${HAIKU_REFUSAL}`);
    return null;
  }
  return value;
}
...
// The deployment default every agent falls back to when it carries no override.
// Claude Opus 5 is the house default: the deliberation prompts are written for
// it, and a preflight that passes on Opus 5 is the shipped-and-verified path.
// MODEL still wins when a deployment sets it.
const DEFAULT_MODEL = 'claude-opus-5';
const MODEL = modelFromEnv('MODEL', process.env.MODEL) || DEFAULT_MODEL;
...
const QUALITY_MODEL = modelFromEnv('QUALITY_MODEL', process.env.QUALITY_MODEL);
...
// A dedicated key for the metered OpenAI platform. OPENAI_API_KEY cannot serve
// that purpose on a deployment that points OPENAI_BASE_URL at a gateway,
// because there it holds the GATEWAY's bearer token, not an OpenAI key.
const OPENAI_PLATFORM_API_KEY = process.env.OPENAI_PLATFORM_API_KEY || null;

// Is the configured default endpoint a self-hosted gateway rather than the
// public OpenAI API? A gateway in this topology fronts a flat-fee subscription
// (cli-proxy-api over a Claude Max / ChatGPT plan), which decides two things:
// the `subscription` route can resolve to it, and its traffic is billed as an
// amortized plan slice instead of metered per-token rates.
function isSelfHostedGateway(url) {
  if (!url) return false;
  try {
    return !/(^|\.)api\.openai\.com$/i.test(new URL(url).host);
  } catch {
    return false;
  }
}
const DEFAULT_ROUTE_IS_GATEWAY = !!(OPENAI_API_KEY && isSelfHostedGateway(OPENAI_BASE_URL));

// What the deployment's own provider costs per token, as a billing mode.
// lib/cost.js stays pure: it is told, it does not guess.
function defaultRouteBilling() {
  return DEFAULT_ROUTE_IS_GATEWAY ? 'amortized' : 'published';
}
...
function routeCreds(route) {
  switch (route) {
    ...
    case 'openai-api': {
      // Only a real platform key belongs on api.openai.com. Falling back to
      // OPENAI_API_KEY is correct ONLY when no gateway is configured, because
      // otherwise that variable is the gateway's bearer token: the route
      // advertised itself as available and then 401'd on every call.
      const key = OPENAI_PLATFORM_API_KEY || (DEFAULT_ROUTE_IS_GATEWAY ? null : OPENAI_API_KEY);
      return key ? { transport: 'openai', baseUrl: 'https://api.openai.com/v1', apiKey: key } : null;
    }
    case 'openrouter':
      return OPENROUTER_API_KEY ? { transport: 'openai', baseUrl: OPENROUTER_BASE_URL, apiKey: OPENROUTER_API_KEY } : null;
    case 'subscription':
      if (CLIPROXY_GATEWAY_URL && CLIPROXY_GATEWAY_TOKEN) {
        return { transport: 'openai', baseUrl: CLIPROXY_GATEWAY_URL, apiKey: CLIPROXY_GATEWAY_TOKEN };
      }
      // No dedicated CLIPROXY_* pair, but the default endpoint is already a
      // subscription gateway. Reporting "no credentials" for the route named
      // `subscription` on a deployment that runs entirely on a subscription is
      // the most misleading thing this function can say.
      return DEFAULT_ROUTE_IS_GATEWAY ? { transport: 'openai', baseUrl: OPENAI_BASE_URL, apiKey: OPENAI_API_KEY } : null;
    case 'ollama-local':
...
// `routingOverride`, when supplied, replaces the stored agent_routing map for
// this resolution only. The preflight dry run needs to resolve a CANDIDATE map
// before it is written; without this it could only ever check what is already
// live, which is the one config that does not need checking.
function resolveRoute(agentId, explicitModel, routingOverride) {
  const cfg = routingOverride
    ? ((routingOverride[agentId] && typeof routingOverride[agentId] === 'object') ? routingOverride[agentId] : null)
    : appConfig.getAgentRoute(agentId);
  // The writes refuse a Haiku model, but a row stored before that rule (or
  // edited by hand) still exists. Treat its model as absent, like the env.
  const storedModel = cfg && cfg.model && !isHaikuModel(cfg.model) ? cfg.model : null;
  const callerModel = explicitModel && !isHaikuModel(explicitModel) ? explicitModel : null;
  if ((cfg && cfg.model && !storedModel) || (explicitModel && !callerModel)) {
    log.warn({ agentId }, `Haiku model ignored: ${HAIKU_REFUSAL}`);
  }
  const model = callerModel || storedModel || resolveModel(agentId);
  if (cfg && cfg.route) {
    ...
    const routeModel = callerModel || storedModel;
...
// Support calls that take QUALITY_MODEL when it is set, and MODEL otherwise.
// Each one can also carry a stored route and model (lib/routable-agents.js).
const SUPPORT_AGENTS = new Set(['quality-evaluator', 'adversarial-twin', 'memory-analyzer', 'fingerprint-classifier']);
...
for (const [key, value] of Object.entries(process.env)) {
  ...
  const model = match ? modelFromEnv(key, value) : null;
  if (model) {
    AGENT_MODEL_OVERRIDES[match[1]] = model;
  }
}
...
function resolveModel(agentId) {
  ...
  // 2. Support agents use QUALITY_MODEL
  if (SUPPORT_AGENTS.has(agentId) && QUALITY_MODEL) return QUALITY_MODEL;
  // 3. Default model
  return MODEL;
}
...
// Minimal tool definition for the `tools:true` probe. Every agent turn ships a
// tools array (escalate_to_human at minimum), so a model that answers a bare
// prompt but rejects `tools` fails every turn while a plain probe stays green.
// The probe only checks that the provider accepts the request; whether the
// model chooses to call the tool is not part of the verdict.
const PROBE_TOOL = {
  name: 'ping',
  description: 'Probe tool. Do not call it.',
  input_schema: { type: 'object', properties: {}, additionalProperties: false },
};
async function testConnection({ route = '', model = '', tools = false, timeoutMs = TEST_CONNECTION_TIMEOUT_MS } = {}) {
  // Refused before any network call: a probe is still a call to the model.
  if (isHaikuModel(model)) return { ok: false, route: route || 'default', model, error: HAIKU_REFUSAL };
  let r;
  if (route) {
  ...
  try {
    if (r.transport === 'openai') {
      const body = { model: resolvedModel, max_tokens: tools ? 16 : 1, messages: [{ role: 'user', content: 'ping' }] };
      if (tools) {
        body.tools = [{ type: 'function', function: { name: PROBE_TOOL.name, description: PROBE_TOOL.description, parameters: PROBE_TOOL.input_schema } }];
      }
      ...
      const client = new Anthropic({ apiKey: r.apiKey, timeout: timeoutMs, maxRetries: 0 });
      const modelId = resolvedModel.startsWith('anthropic/') ? resolvedModel.slice('anthropic/'.length) : resolvedModel;
      const req = { model: modelId, max_tokens: tools ? 16 : 1, messages: [{ role: 'user', content: 'ping' }] };
      if (tools) req.tools = [PROBE_TOOL];
      await client.messages.create(req);
    } else {
      throw new Error('No LLM configuration available');
    }
    return { ok: true, route: r.route, model: resolvedModel, tools: !!tools, latencyMs: Date.now() - started };
  } catch (err) {
    ...
    return { ok: false, route: r.route, model: resolvedModel, tools: !!tools, latencyMs: Date.now() - started, error: msg };
  }
}
...
// Claude Haiku ids are left out, so neither the panel nor an MCP client can
// offer one.
...
async function listModels({ route = '' } = {}) {
  ...
  try {
    ...
    return { ok: true, route: r.route, models: [...new Set(ids)].filter(id => !isHaikuModel(id)).sort() };
  } catch (err) {
    const msg = String(err && err.message || err).slice(0, 300);
...
module.exports = { callAnthropic, callAnthropicWithTools, callLLMRaw, resolveModel, resolveRoute, availableRoutes, testConnection, listModels, defaultRouteBilling, isSelfHostedGateway, QUALITY_MODEL, DEFAULT_MODEL, MODEL, AGENT_MAX_TOKENS, REASONING_FLOOR, parseAgentMaxTokens, anyLLMInFlight, inFlightLLMCount };
