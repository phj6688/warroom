...
// turn, its phase, and the whole session behind it. On 2026-08-12 a single
// request held session 3fe5a74f open for more than two hours with no log line,
// no error row, and no phase advance.
...
function isTimeoutError(err) {
  ...
  return String((err && err.message) || '').startsWith(TIMEOUT_MESSAGE);
}
...
// AbortSignal.timeout() is deliberately not used: its timer is unref'd, so it
// cannot fire in an otherwise idle process. An explicit controller matches the
// files-service and search clients and always fires.
...
async function _openaiChat(route, body) {
  ...
  let res = await _openaiPost(url, route.apiKey, body);
  if (res.ok) return res.json();

  const text = await res.text();
  const afford = res.status === 402 ? Number((AFFORDABLE_TOKENS_RE.exec(text) || [])[1]) : NaN;
  if (Number.isFinite(afford) && afford > 0 && (body.max_tokens == null || afford < body.max_tokens)) {
    log.warn({ afford, model: body.model }, 'provider credit ceiling; retrying once at the affordable budget');
    res = await _openaiPost(url, route.apiKey, { ...body, max_tokens: afford });
...
async function _callOnce(systemPrompt, messages, agentId, maxTokens, onUsage = null) {
  ...
  if (r.transport === 'anthropic') {
    const Anthropic = require('@anthropic-ai/sdk');
    const client = new Anthropic({ apiKey: r.apiKey, timeout: LLM_TIMEOUT_MS });
