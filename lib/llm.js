...
function resolveRoute(agentId, explicitModel) {
  ...
  if (cfg && cfg.route) {
    const creds = routeCreds(cfg.route);
    if (creds) return { route: cfg.route, model, ...creds };
    log.warn({ agentId, route: cfg.route }, 'configured route has no credentials; falling back to default');
  }
...
module.exports = { callAnthropic, callAnthropicWithTools, callLLMRaw, resolveModel, resolveRoute, availableRoutes, QUALITY_MODEL, anyLLMInFlight, inFlightLLMCount };
