...
async function callAnthropic(systemPrompt, messages, agentId, maxTokens = 1500, onUsage = null) {
  const tracker = (async () => {
    let lastError;
...
async function _callOnceWithTools(systemPrompt, messages, agentId, tools, maxTokens) {
  ...
  if (OPENAI_BASE_URL && OPENAI_API_KEY) {
    ...
    return { text, toolCalls, usage: normalizeUsage(data.usage) };
  }
  if (ANTHROPIC_API_KEY) {
    const Anthropic = require('@anthropic-ai/sdk');
    const client = new Anthropic({ apiKey: ANTHROPIC_API_KEY });
    ...
    return { text, toolCalls, usage: normalizeUsage(response.usage) };
  }
  throw new Error('No LLM configuration available');
}
...
async function _callLLMRawOnce({ model, system, messages, tools, maxTokens, agentId }) {
  if (OPENAI_BASE_URL && OPENAI_API_KEY) {
    ...
    return { stop_reason, content, usage: normalizeUsage(data.usage) };
  }
  if (ANTHROPIC_API_KEY) {
    const Anthropic = require('@anthropic-ai/sdk');
    const client = new Anthropic({ apiKey: ANTHROPIC_API_KEY });
    ...
    return { stop_reason: response.stop_reason, content: response.content || [], usage: normalizeUsage(response.usage) };
  }
  throw new Error('No LLM configuration available');
}
