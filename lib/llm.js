...
async function _callOnce(systemPrompt, messages, agentId, maxTokens) {
  ...
  if (ANTHROPIC_API_KEY) {
    ...
    const response = await client.messages.create({ model: modelId, max_tokens: 16000, system: systemPrompt, messages, thinking: { type: "enabled", budget_tokens: 10000 } });
...
async function _callOnceWithTools(systemPrompt, messages, agentId, tools, maxTokens) {
  ...
  if (ANTHROPIC_API_KEY) {
    ...
    const client = new Anthropic({ apiKey: ANTHROPIC_API_KEY });
    const modelId = model.startsWith('anthropic/') ? model.slice('anthropic/'.length) : model;
    const req = { model: modelId, max_tokens: 16000, system: systemPrompt, messages, thinking: { type: "enabled", budget_tokens: 10000 } };
...
async function callLLMRaw({ model, system, messages, tools, maxTokens, agentId } = {}) {
  const resolvedModel = model || resolveModel(agentId);

  const tracker = (async () => {
    let lastError;
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        return await _callLLMRawOnce({ model: resolvedModel, system, messages, tools, maxTokens, agentId });
      } catch (err) {
        lastError = err;
        if (attempt < MAX_RETRIES - 1) {
          await new Promise(r => setTimeout(r, BACKOFF_MS[attempt]));
...
async function _callLLMRawOnce({ model, system, messages, tools, maxTokens, agentId }) {
  if (GATEWAY_URL && GATEWAY_TOKEN) {
    ...
    return { stop_reason, content, usage: data.usage };
  }
  if (ANTHROPIC_API_KEY) {
    const Anthropic = require('@anthropic-ai/sdk');
    const client = new Anthropic({ apiKey: ANTHROPIC_API_KEY });
    const modelId = model.startsWith('anthropic/') ? model.slice('anthropic/'.length) : model;
    const req = { model: modelId, max_tokens: 16000, system, messages, thinking: { type: "enabled", budget_tokens: 10000 } };
    if (tools && tools.length > 0) req.tools = tools;
    const response = await client.messages.create(req);
    if (BASELINE_LOG && response.usage) {
      const entry = JSON.stringify({ agent: agentId, model: modelId, usage: response.usage, ts: Date.now() });
    ...
    return { stop_reason: response.stop_reason, content: (response.content || []).filter(b => b.type !== 'thinking'), usage: response.usage };
  }
  throw new Error('No LLM configuration available');
}

module.exports = { callAnthropic, callAnthropicWithTools, callLLMRaw, resolveModel, QUALITY_MODEL, anyLLMInFlight, inFlightLLMCount };
