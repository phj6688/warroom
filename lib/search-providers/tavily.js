...
function createTavilyProvider({ apiKey, maxResults = 5, logger, timeoutMs, fetch: fetchImpl } = {}) {
  ...
  // A default parameter would only cover `undefined`, so an explicit 0 or null
  // (which used to mean "no deadline") would become "abort immediately".
  const deadlineMs = timeoutMs == null ? DEFAULT_TIMEOUT_MS : timeoutMs;
