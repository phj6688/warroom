...
async function runWithTools({
  ...
  for (let iter = 0; iter < LOOP_SAFETY; iter++) {
    rounds += 1;
    const t0 = Date.now();
    // agentId is what lib/llm.js resolves the provider route from. Drop it and
    // every search-enabled agent silently falls back to the env default route
    // instead of the one configured for it in Settings.
    const response = await llmCall({ model, system, messages: working, tools, maxTokens, agentId });
