...
function resumeSession(deps, sessionId) {
  ...
  // Clear how the last attempt ended. Leaving it would report a running room
  // as "Outcome: stopped", and boot reconciliation's COALESCE would then
  // preserve that stale label if this attempt crashed too.
  stmts.updateSessionOutcome.run(null, null, Date.now(), session.id);
