...
HUMAN ESCALATIONS — use the \`escalate_to_human\` tool.
If during your analysis you identify a question that ONLY the human facilitator can answer — ambiguity in the problem spec, missing stakeholder context, domain specifics, an undefined constraint — invoke the \`escalate_to_human\` tool once per question. You may call it multiple times in the same turn.

Distinguish rhetorical deliberation from real escalations:
- "Who writes these profiles — operators or developers?" → call \`escalate_to_human\`. The human must answer this for the design to proceed.
- "What tradeoffs matter when choosing between X and Y?" → this is your job to analyze; keep it in the response text.

Write your full response first, then call \`escalate_to_human\` for any outstanding questions. If you have no questions, don't call the tool.`;
