...
function buildContext(session, agentId, phase) {
  ...
  if (initialUtilization > 0.9) {
    ...
    while (trimmed.length > 0) {
      ...
      if (rebuiltTotal <= budget * 0.8) {
        ...
        console.warn(`[tokens] context trimmed for ${agentId}: dropped ${droppedCount} oldest priors (${totalTokens} → ${rebuiltTotal} tokens)`);
    ...
    if (systemTokens + countTokens(bare) <= budget * 0.8) {
      console.warn(`[tokens] context trimmed for ${agentId}: dropped ALL ${priorMessageObjs.length} priors`);
  ...
  if (initialUtilization > 0.8) {
    console.warn(`[tokens] WARNING: context for ${agentId} at ${(initialUtilization * 100).toFixed(0)}% of budget (${totalTokens}/${budget} tokens)`);
  }
