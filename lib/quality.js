...
function createQualityManager(deps) {
  const { db, stmts, callAnthropic, PHASES } = deps;
  ...
  async function generateShadowAnswer(sessionId, problem) {
    if (!problem || problem.trim().length < 20) {
      console.log(`[quality] Skipping shadow for ${sessionId} — problem too short`);
      return null;
    }
    ...
    try {
      ...
      console.log(`[quality] Shadow answer generated for ${sessionId} (${countTokens(answer)} tokens)`);
      return answer;
    } catch (err) {
      console.warn(`[quality] Shadow generation failed for ${sessionId}: ${err.message}`);
      return null;
    }
  }
  ...
  async function computeSynthesisStructureScore(sessionId) {
    ...
    try {
      ...
      console.warn(`[quality] Structure scoring failed for ${sessionId}: ${err.message}`);
  ...
  async function computeShadowDelta(sessionId) {
    ...
    try {
      ...
      console.warn(`[quality] Shadow delta scoring failed for ${sessionId}: ${err.message}`);
      return null;
    }
  }
  ...
  async function evaluateSession(sessionId) {
    ...
    if (existing) {
      console.log(`[quality] Session ${sessionId} already evaluated, skipping`);
      return getQualityScore(sessionId);
    }

    console.log(`[quality] Evaluating session ${sessionId}...`);
    ...
    console.log(`[quality] Session ${sessionId} scored: ${finalScore.toFixed(3)} (phase=${phaseCompletion.toFixed(2)}, esc=${escalationEfficiency.toFixed(2)}, struct=${structureScore.toFixed(2)}, refs=${crossRefCount}, shadow=${shadowDelta !== null ? shadowDelta.toFixed(2) : 'null'})`);
  ...
  async function retroactiveScore() {
    ...
    if (sessions.length === 0) {
      console.log('[quality] No sessions need retroactive scoring');
      return { scored: 0, failed: 0 };
    }

    console.log(`[quality] Retroactively scoring ${sessions.length} sessions...`);
    let scored = 0;
    let failed = 0;
    ...
    for (const { id } of sessions) {
      try {
        ...
        console.warn(`[quality] Retroactive scoring failed for ${id}: ${err.message}`);
        failed++;
      }
    }

    console.log(`[quality] Retroactive scoring complete: ${scored} scored, ${failed} failed`);
