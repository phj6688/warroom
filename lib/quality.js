...
const { resolveRoute } = require('./llm');
...
function createQualityManager(deps) {
  ...
  async function computeSynthesisStructureScore(sessionId) {
    ...
    // Truncate if too long for the evaluator call
  ...
  async function evaluateSession(sessionId) {
    ...
    // B7 (HLB-797) — a run that did not reach the end must never be scored:
    // its composite would pollute the metric with an infrastructure failure,
    // and the score makes an interrupted session look judged. NULL is a legacy
    // row from before the outcome column and is treated as complete. This
    // guards direct callers; the completion path and retroactiveScore already
    // exclude them.
    if (session.outcome && session.outcome !== 'complete') {
      log.info({ sessionId, outcome: session.outcome }, 'quality: run did not complete, skipping scoring');
      return null;
    }
    ...
    // The model the evaluator call resolves to, so a stored support route or a
    // skipped env value is recorded as what ran, not as what env said. Resolved
    // here, in the same tick as the first evaluator call resolves its own route,
    // so a routing change while the scoring awaits cannot relabel the row.
    const model = resolveRoute('quality-evaluator').model || 'unknown';
  ...
  function getAnalytics() {
    // HLB-880 — only score genuine deliberations. Exclude any quality_scores row
    // whose session produced no Synthesis message: a failed run (B7) is never
    // scored going forward, but historical failed/empty sessions carry rows that
    // otherwise drag the average down with infrastructure failures.
    const rows = db.prepare(
      "SELECT composite_score, phase_completion_rate, escalation_efficiency, synthesis_structure_score, cross_ref_count, shadow_delta, created_at FROM quality_scores q WHERE EXISTS (SELECT 1 FROM messages m WHERE m.session_id = q.session_id AND m.phase = 'Synthesis') ORDER BY created_at ASC"
    ).all();
    ...
    const scored = db.prepare(
      "SELECT q.session_id, q.composite_score, s.problem FROM quality_scores q JOIN sessions s ON s.id = q.session_id WHERE EXISTS (SELECT 1 FROM messages m WHERE m.session_id = q.session_id AND m.phase = 'Synthesis') ORDER BY q.composite_score DESC"
    ).all();
  ...
  async function retroactiveScore() {
    // Only a run that reached the end is scored. A NULL outcome is a legacy
    // row from before the column existed and is treated as complete; the
    // named non-complete states (failed, stopped, crashed) are not scored,
    // because a score on a dead session makes it look judged rather than
    // interrupted.
    const sessions = db.prepare(
      "SELECT id FROM sessions WHERE active = 0 AND (outcome IS NULL OR outcome = 'complete') AND id NOT IN (SELECT session_id FROM quality_scores)"
    ).all();
