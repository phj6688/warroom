...
function createQualityManager(deps) {
  ...
  async function evaluateSession(sessionId) {
    ...
    // B7 (HLB-797) — a failed run (every agent turn errored) must never be
    // scored: its composite would pollute the metric with an infrastructure
    // failure. This guards direct callers; the completion path and
    // retroactiveScore already exclude failed sessions.
    if (session.outcome === 'failed') {
      log.info({ sessionId }, 'quality: outcome=failed, skipping scoring');
      return null;
    }
  ...
  async function retroactiveScore() {
    const sessions = db.prepare(
      "SELECT id FROM sessions WHERE active = 0 AND (outcome IS NULL OR outcome != 'failed') AND id NOT IN (SELECT session_id FROM quality_scores)"
    ).all();
