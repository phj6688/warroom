...
function createSpecialistSpawner(deps) {
  ...
  function spawnSpecialists(domains, limit = MAX_SPECIALISTS) {
    ...
    for (const domain of domains.slice(0, limit)) {
      ...
      // Append the web-search capability fragment for tier-C specialists
      // when the expansion flag is on. Truncation runs first so the
      // fragment stays intact.
      const searchConfig = getSearchConfigForAgent(template.id);
      if (searchConfig) prompt = appendSearchFragment(prompt, searchConfig);
    ...
    if (agents.length > 0) {
      log.info({ count: agents.length, names: agents.map(a => a.name) }, 'specialists spawned');
    }
